    ...
    pub fn to_standard_notation_string(&self) -> String {
        ...
        let (sourlo, sourml, sourmh, sourhi) = if cfg!(target_endian = "little") {
            (
                ...
                u32::from_le_bytes(self.inner.bytes[8..12].try_into().unwrap()) as usize,
                u32::from_le_bytes(self.inner.bytes[12..16].try_into().unwrap()) as usize,
            )
        } else {
            (
                u32::from_le_bytes(self.inner.bytes[12..16].try_into().unwrap()) as usize,
        ...
        dpd2char!(dpd, u, digits, digits_idx);
